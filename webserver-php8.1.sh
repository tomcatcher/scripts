...
echo "deb https://packages.sury.org/php/ bookworm main" > /etc/apt/sources.list.d/php.list && echo "Repo list writen" \
        || echo "Could not write /etc/apt/sources.list.d/php.list" && exit 1
apt -y install lsb-release ca-certificates curl wget gnupg2 > /dev/null 2>&1 && \
        echo "Prerequisites installed" || echo "Could not install prerequisites" && exit 2
wget -qO- https://packages.sury.org/php/apt.gpg | gpg --dearmor > /etc/apt/trusted.gpg.d/sury-php-x.x.gpg && \
        echo "Key for repo set up" || echo "Could not set up php 8.1 repo" && exit 3
apt update && echo "APT refreshed" || echo "Could not refresh APT" && exit 3
apt -y install apache2 apache2-bin apache2-data apache2-doc apache2-utils fontconfig-config fonts-dejavu-core \
               ...
               php8.1-readline php8.1-xml php8.1-zip && echo "Packages installed" || echo "Could not install packages" && exit 4
echo "Apache site configuration will open for editing once You press ENTER"
read
nano /etc/apache2/sites-available/000-default.conf
# a2enmod rewrite
# a2enmod headers
systemctl restart apache2 && echo "Apache restarted" || echo "Could not restart apache" && exit 5
